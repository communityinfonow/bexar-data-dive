<template>
  <v-container fluid>
    ...
    <h2 class="text-h5 mb-2">{{ $t('home_view.available_tools') }}</h2>
    <section class="d-flex flex-row">
      ...
      >
        <v-img src="@/assets/todo.jpg" class="white--text align-end">
        ...
        <v-card-actions>
          ...
          <v-spacer></v-spacer>
          <v-btn color="primary" :to="tool.route" :aria-label="$t('home_view.view') + ' ' + tool.name">{{
            $t('home_view.view')
          }}</v-btn>
        </v-card-actions>
      </v-card>
